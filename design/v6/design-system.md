# Design System v6 — Party Cupid (Unified, Final)

> **v5 → v6 변경** (90점 돌파 정밀 수정):
> 1. 안전 정책을 "UI가 약속하지 않는" 원칙으로 재정리 (3회 누적 자동 검토 제거)
> 2. 충돌 모달 "둘 다 취소" → "**제자리 유지 (나중에 결정)**"로 변경 (맥락 유지)
> 3. A-7 필터를 탐색 어휘로 재편 (편집 어휘는 A-9 전담)
> 4. ROUND_BANNER 상하 동시 표시 시 **aria-live는 단 1개만 활성** (나머지 off)
> 5. 충돌+마감 동시 발생 우선순위 규칙 신규
