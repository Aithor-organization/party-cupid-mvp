# Design System v7 — Party Cupid (Unified, Final+)

> **v6 → v7 변경** (90점 돌파 최종 수정):
> 1. A-7 카드 배지 탐색어 통일 ("선택됨" / 🆕 · 편집어는 A-9 전담)
> 2. "새로 들어옴" 기준 **마지막 방문 이후 5분 내 입장자** 명확화 + 툴팁
> 3. Save State Tokens 추가 — `local-pending` / `server-final` / `conflict-pending`
> 4. 모든 **수치는 서버 확정 기준**, pending은 별도 ⏳ 뱃지
> 5. 충돌 모달 **ESC 허용** → "제자리 유지" 처리 (접근성 탈출 경로)
